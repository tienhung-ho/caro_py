...
class screen(tk.Tk):
    ...
    def menu_main(self, Ox, Oy):
        bg = ImageTk.PhotoImage(file="jungle2.jpg")
        self.Ox = Ox
        self.Oy = Oy
        # Create a Canvas
        ...
        image = PIL.Image.open("jungle2.jpg")
    ...
    def menu_bar(self, Ox, Oy):
        ...
        file_menu.add_command(label='New...', command=partial(self.newgame, Ox, Oy))
    ...
    def dislay(self, Ox=10, Oy=10):
        ...
        my_Entry = Entry(frame_Menu, width=30, borderwidth=4, relief=tk.SUNKEN)
        my_Entry.grid(row=1, column=3, columnspan=5)
        ...
        my_Label2.grid(row=1, column=30)
        for x in range(Ox):
            for y in range(Oy):
    ...
    def mess_box(self, Ox, Oy, status):
        ...
        # open image to resize it
        image1 = PIL.Image.open("player1-win.png")
        # resize the image with width and height of root
        ...
        # open image to resize it
        image2 = PIL.Image.open("player2-win.jpg")
        # resize the image with width and height of root
